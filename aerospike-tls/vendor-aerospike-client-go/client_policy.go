...
func (cp *ClientPolicy) serviceString() string {
	// PATCH (e2e-tls-run): always request the clear-text service info
	// command. Upstream Aerospike is CE; service-tls-std is EE-only
	// and returns "ERROR:25:enterprise only", which breaks node
	// validation. TLS is still used on the wire (stunnel terminates
	// in front of CE); only the info command name is overridden.
	if cp.UseServicesAlternate {
...
func (cp *ClientPolicy) peersString() string {
	// PATCH (e2e-tls-run): same reason — CE doesn't answer
	// peers-tls-std. Ask peers-clear-std even on a TLS connection.
	if cp.UseServicesAlternate {
